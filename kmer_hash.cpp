...
int main(int argc, char** argv) {
    ...
    UpcHashMap hashmap(hash_table_size);
    ...
    for (const auto& start_kmer : start_nodes) {
        ...
        while (contig.back().forwardExt() != 'F') {
            kmer_pair kmer;
            bool success = hashmap.find(contig.back().next_kmer(), kmer).wait();
            //kmer = hashmap.find(contig.back().next_kmer())
            if (!success) {
    ...
    //TODO: rewrite list to iterate through the contigs with futures
